...
function DispatchForm({ batch, navigate }) {
  ...
  if (dispatched) {
    return (
      <div style={{ background: "#f0fdf4", borderRadius: "12px", padding: "24px", textAlign: "center" }}>
        <div style={{ fontSize: "48px", marginBottom: "12px" }}>🚚</div>
        <div style={{ fontWeight: "bold", color: "#16a34a", fontSize: "18px", marginBottom: "8px" }}>
          Batch Successfully Dispatched!
        </div>
        <div style={{ color: "#64748b", fontSize: "14px", marginBottom: "4px" }}>Vehicle: {vehicle}</div>
        <div style={{ color: "#64748b", fontSize: "14px", marginBottom: "16px" }}>Destination: {destination}</div>
        <div style={{ fontSize: "13px", color: "#94a3b8" }}>
          Dispatched on {new Date().toLocaleDateString()}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "#f8fafc", borderRadius: "12px", padding: "24px", border: "1px solid #e2e8f0" }}>
      <div style={{ fontWeight: "bold", fontSize: "16px", marginBottom: "8px", color: "#0f172a" }}>
        🚚 Ready to Dispatch
      </div>
      <div style={{ color: "#64748b", fontSize: "14px", marginBottom: "20px" }}>
        QA checks are complete. Fill dispatch details to release this batch.
      </div>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <input
          ...
          placeholder="Central Warehouse, Hyderabad"
          style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #e2e8f0", fontSize: "14px", boxSizing: "border-box" }}
        />
      </div>
      ...
      <button
        onClick={() => {
          if (!vehicle || !destination) {
            alert("Please fill vehicle number and destination");
            return;
          }
          setDispatched(true);
        }}
        style={{ width: "100%", padding: "14px", background: "#16a34a", color: "white", border: "none", borderRadius: "12px", cursor: "pointer", fontWeight: "bold", fontSize: "15px" }}>
        ✅ Confirm Dispatch
      </button>
    </div>
  );
...
export default function BatchDetail() {
  ...
  const tabs = user.role === "Supervisor"
    ? ["Details", "Materials", "Approvals"]
    : user.role === "Operator"
    ? ["Details", "QA Checks"]
    : ["Details", "Materials", "QA Checks", "Approvals"];
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        ...
        {activeTab === "Details" && (
          <div>
            ...
            {/* Ingredient Hold Warning */}
            {hasIngredientOnHold && (
              <div style={{ marginTop: "16px", background: "#fffbeb", border: "1px solid #fbbf24", borderRadius: "12px", padding: "16px", display: "flex", gap: "12px", alignItems: "flex-start" }}>
                <div style={{ fontSize: "24px" }}>⚠️</div>
                <div>
                  <div style={{ fontWeight: "bold", color: "#ca8a04", marginBottom: "4px" }}>Batch on Hold — Ingredient Under Review</div>
                  <div style={{ fontSize: "14px", color: "#475569" }}>One or more raw ingredients are currently under review. This batch cannot be released until all ingredients are approved.</div>
                  <div style={{ marginTop: "10px" }}>
                    {ingredients.filter((ing) => ing.status === "Under Review").map((ing) => (
            ...
            {batch.status === "QA Rejected" && batch.rejectionReason && (
              <div style={{ marginTop: "16px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "12px", padding: "16px", display: "flex", gap: "12px", alignItems: "flex-start" }}>
                <div style={{ fontSize: "24px" }}>❌</div>
                <div>
                  <div style={{ fontWeight: "bold", color: "#dc2626", marginBottom: "8px", fontSize: "16px" }}>Why was this batch QA Rejected?</div>
                  <div style={{ fontSize: "14px", color: "#475569", marginBottom: "10px" }}>{batch.rejectionReason}</div>
                  <div style={{ fontSize: "13px", color: "#94a3b8" }}>
        ...
        {activeTab === "Materials" && (
          <div style={{ background: "white", borderRadius: "16px", padding: "24px" }}>
            ...
            {ingredients.map((ing) => (
              <div key={ing.id}>
                <div style={{
                  padding: "16px", borderRadius: "12px", background: "#f8fafc", marginBottom: "8px",
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    {ing.status === "Under Review" && (
                      <button
                        ...
                        style={{ padding: "6px 14px", borderRadius: "8px", border: "none", background: "#fef9c3", color: "#ca8a04", cursor: "pointer", fontSize: "13px", fontWeight: "bold" }}>
                ...
                {expandedIngredient === ing.id && ingredientReviewReasons[ing.id] && (
                  <div style={{ background: "#fffbeb", border: "1px solid #fbbf24", borderRadius: "12px", padding: "16px", marginBottom: "8px", marginLeft: "8px" }}>
                    <div style={{ fontWeight: "bold", color: "#ca8a04", marginBottom: "10px" }}>⚠️ Why is this Under Review?</div>
                    <div style={{ fontSize: "14px", color: "#475569", marginBottom: "6px" }}><strong>Issue:</strong> {ingredientReviewReasons[ing.id].reason}</div>
                    <div style={{ fontSize: "14px", color: "#475569", marginBottom: "6px" }}><strong>Detected by:</strong> {ingredientReviewReasons[ing.id].detectedBy}</div>
                    <div style={{ fontSize: "14px", color: "#475569", marginBottom: "6px" }}><strong>Date:</strong> {ingredientReviewReasons[ing.id].date}</div>
                    <div style={{ fontSize: "14px", color: "#475569" }}><strong>Action:</strong> {ingredientReviewReasons[ing.id].action}</div>
                  </div>
                )}
              </div>
            ))}
        ...
        {activeTab === "QA Checks" && (
          <div style={{ background: "white", borderRadius: "16px", padding: "24px" }}>
            <h2 style={{ margin: "0 0 8px 0" }}>✅ QA Checklist</h2>

            <div style={{
              background: user.role === "Operator" ? "#eff6ff" : "#f0fdf4",
              borderRadius: "10px", padding: "12px 16px", marginBottom: "20px", fontSize: "14px",
              color: user.role === "Operator" ? "#2563eb" : "#16a34a"
            }}>
              {user.role === "Operator"
                ? "📱 Upload photo evidence for each check below"
                : "🔍 Review photo evidence and mark each check as Pass or Fail"}
            </div>
              ...
              <div key={item} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button onClick={() => handleStatus(item, "pass")}
                      style={{ padding: "8px 20px", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "bold", background: data.status === "pass" ? "#16a34a" : "#e2e8f0", color: data.status === "pass" ? "white" : "#475569" }}>
                      ✓ Pass
                    </button>
                    <button onClick={() => handleStatus(item, "fail")}
                      style={{ padding: "8px 20px", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "bold", background: data.status === "fail" ? "#dc2626" : "#e2e8f0", color: data.status === "fail" ? "white" : "#475569" }}>
                      ✗ Fail
                    </button>
                  </div>
                </div>

                {/* Photo section - different for Operator vs QA Manager */}
                <div style={{ marginTop: "8px" }}>
                  {user.role === "Operator" ? (
                    <label style={{ padding: "8px 16px", background: "white", border: "1px dashed #cbd5e1", borderRadius: "8px", cursor: "pointer", fontSize: "13px", color: "#64748b", display: "inline-flex", alignItems: "center", gap: "6px" }}>
                      📷 Upload Photo Evidence
                      <input type="file" accept="image/*" style={{ display: "none" }} onChange={(e) => handlePhoto(item, e)} />
                    </label>
                  ) : (
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <div style={{ width: "70px", height: "70px", borderRadius: "10px", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px" }}>
                        📸
                      </div>
                      <div>
                        <div style={{ fontSize: "13px", color: "#16a34a", fontWeight: "500" }}>✅ Photo uploaded by Operator</div>
                        <div style={{ fontSize: "12px", color: "#94a3b8" }}>Tap to view full image</div>
                      </div>
                    </div>
                  )}

                  {data.photo && data.photo.startsWith("blob") && (
                    <img src={data.photo} alt="evidence" style={{ width: "70px", height: "70px", borderRadius: "8px", objectFit: "cover", marginTop: "8px", border: "2px solid #e2e8f0" }} />
                  )}

                  {data.status && (
                    <div style={{ fontSize: "13px", fontWeight: "bold", color: data.status === "pass" ? "#16a34a" : "#dc2626", marginTop: "8px" }}>
                ...
                {data.status === "fail" && (
                  <div style={{ marginTop: "12px" }}>
                    <textarea
                      ...
                      rows={2}
                      style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", border: "1px solid #fecaca", fontSize: "14px", background: "#fff5f5", color: "#dc2626", boxSizing: "border-box", resize: "vertical" }}
                    />
                    {data.reason && <div style={{ fontSize: "12px", color: "#dc2626", marginTop: "4px" }}>⚠️ Reason recorded</div>}
                  </div>
                )}
              </div>
            ))}
            ...
            {allDone && (
              <div style={{ marginTop: "24px", padding: "20px", borderRadius: "12px", background: allPassed ? "#f0fdf4" : "#fef2f2", border: `1px solid ${allPassed ? "#bbf7d0" : "#fecaca"}` }}>
                ...
                {anyFailed && (
                  <div style={{ marginTop: "16px", borderTop: "1px solid #fecaca", paddingTop: "16px" }}>
                    ...
                    {Object.entries(qaChecks).filter(([, d]) => d.status === "fail").map(([item, d]) => (
                      <div key={item} style={{ background: "white", borderRadius: "8px", padding: "10px 14px", marginBottom: "8px", border: "1px solid #fecaca" }}>
                        ...
                        {d.photo && <img src={d.photo} alt="evidence" style={{ width: "80px", height: "80px", borderRadius: "8px", objectFit: "cover", marginTop: "8px", border: "2px solid #fecaca" }} />}
                        {d.reason ? (
          ...
          <div style={{ background: "white", borderRadius: "16px", padding: "24px" }}>
            ...
            {approvals.map((approval) => (
              <div key={approval.stage} style={{ display: "flex", gap: "16px", alignItems: "flex-start", marginBottom: "20px" }}>
                <div style={{ width: "36px", height: "36px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", flexShrink: 0, background: approval.done ? "#dcfce7" : "#f1f5f9" }}>
                  {approval.done ? "✅" : "⏳"}
                </div>
                <div>
            ...
            {/* Supervisor sees dispatch form */}
            {user.role === "Supervisor" && (
              <div style={{ marginTop: "24px" }}>
                {hasIngredientOnHold ? (
                  <div style={{ background: "#fffbeb", borderRadius: "12px", padding: "20px", textAlign: "center" }}>
                    <div style={{ fontSize: "32px", marginBottom: "8px" }}>🔒</div>
                    <div style={{ fontWeight: "bold", color: "#ca8a04", fontSize: "16px" }}>Cannot Dispatch</div>
                    <div style={{ color: "#64748b", fontSize: "14px", marginTop: "4px" }}>Milk Powder is still Under Review</div>
                  </div>
                ) : batch.status === "QA Rejected" ? (
                  <div style={{ background: "#fef2f2", borderRadius: "12px", padding: "20px", textAlign: "center" }}>
                    <div style={{ fontSize: "32px", marginBottom: "8px" }}>❌</div>
                    <div style={{ fontWeight: "bold", color: "#dc2626", fontSize: "16px" }}>Cannot Dispatch</div>
                    <div style={{ color: "#64748b", fontSize: "14px", marginTop: "4px" }}>This batch was QA Rejected</div>
                  </div>
                ) : batch.status === "Released" ? (
                  <div style={{ background: "#f0fdf4", borderRadius: "12px", padding: "20px" }}>
                    <div style={{ textAlign: "center", marginBottom: "20px" }}>
                      <div style={{ fontSize: "32px", marginBottom: "8px" }}>🚚</div>
                      <div style={{ fontWeight: "bold", color: "#16a34a", fontSize: "16px" }}>Batch Released & Dispatched</div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                      {[
                        { label: "Dispatched By", value: "Sarah Johnson" },
                        { label: "Dispatch Date", value: "Jan 16, 2024" },
                        { label: "Vehicle Number", value: "TS 09 AB 1234" },
                        { label: "Destination", value: "Central Warehouse, Hyderabad" },
                      ].map((field) => (
                        <div key={field.label} style={{ background: "white", borderRadius: "10px", padding: "12px 16px" }}>
                          <div style={{ fontSize: "12px", color: "#94a3b8", marginBottom: "4px" }}>{field.label}</div>
                  ...
                  <DispatchForm batch={batch} navigate={navigate} />
                )}
              </div>
            )}

            {/* Everyone else sees released status */}
            {user.role !== "Supervisor" && (
              <>
                {hasIngredientOnHold ? (
                  <div style={{ marginTop: "24px", background: "#fffbeb", borderRadius: "12px", padding: "20px", textAlign: "center" }}>
                    ...
                    <div style={{ color: "#64748b", fontSize: "14px", marginTop: "4px" }}>Cannot release — Milk Powder is still Under Review</div>
                  </div>
                ) : batch.status === "Released" ? (
